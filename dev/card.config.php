...
if (isset($_POST['action'])) {
    if($_POST['action'] == 'load_config') {
        $stmt = $connection->prepare(
            'SELECT * FROM card_config WHERE id=? LIMIT 1'
        );
        $stmt->bind_param('s', $_POST['card_config_id']);
        $stmt->execute();
        $result = $stmt->get_result();
        if($config = $result->fetch_assoc()) {
            $_POST['p_card_id'] = $config['fk_card_id '];
            $_POST['p_card'] = $config['fk_cardstack_id'];
        ...
        $is_update = isset($_POST['card_config_id']) && $_POST['card_config_id'] != '';
        $content_label = null;
        $stmt = $connection->prepare(
            'SELECT table_name FROM p_cardstack WHERE id=?'
        );
        $stmt->bind_param('s', $_POST['p_cardstack_id']);
        $stmt->execute();
        $result = $stmt->get_result();
        if($plugin = $result->fetch_assoc()) {
            ...
            $resultb = $connection->query(
                "SELECT label FROM " . $table_name . " WHERE id='" . $_POST['p_cardstack_id'] . "'"
            );
        ...
        $stmt->execute();
    } elseif($_POST['action'] == 'delete' && isset($_POST['card_config_id']) && $_POST['card_config_id'] != '') {
        $stmt = $connection->prepare(
            'DELETE FROM card_config WHERE id=?'
        );
        $stmt->bind_param('s', $_POST['card_config_id']);
        $stmt->execute();
        unset($_POST);
